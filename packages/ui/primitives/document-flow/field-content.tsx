...
export const FieldContent = ({ field, documentMeta }: FieldIconProps) => {
  ...
  if (field.type === FieldType.CHECKBOX && field.fieldMeta?.type === 'checkbox') {
    ...
    const useCustomPositioning =
      ...
      field.fieldMeta.values?.every((item) => item.offsetX !== undefined && item.offsetY !== undefined);
      ...
      <div
        className={cn(
          ...
          useCustomPositioning
            ? 'relative'
            : cn('flex', field.fieldMeta.direction === 'horizontal' ? 'flex-row flex-wrap' : 'flex-col gap-y-1'),
        )}
      >
    ...
    return (
      <div className={cn('py-0.5', useCustomPositioning ? 'relative' : 'flex flex-col gap-y-2')}>
        <RadioGroup
          ...
          className={cn(useCustomPositioning ? 'relative h-full w-full' : 'gap-y-1')}
        >
          {field.fieldMeta.values.map((item, index) => (
            <div
              key={index}
