...
const execFileAsync = (cmd: string, args: string[], options: { timeout?: number }): Promise<string> =>
...
export const findBinary = async (name: string): Promise<string> => {
  ...
  const instructions = INSTALL_INSTRUCTIONS[name] ?? `Install ${name} and ensure it is in your PATH.`;
