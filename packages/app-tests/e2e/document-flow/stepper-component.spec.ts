...
test('[DOCUMENT_FLOW]: should not be able to create a document without signatures', async ({ page }) => {
  ...
  await expect(missingSignatureDialog.getByText('No fields assigned to signer').first()).toBeVisible();
...
test('[DOCUMENT_FLOW]: should be able to create, send with redirect url, sign a document and redirect to redirect url', async ({
  ...
  await expect(async () => {
    ...
    await expect(
      approvalDialog.getByText('You are about to complete approving the following document').first(),
