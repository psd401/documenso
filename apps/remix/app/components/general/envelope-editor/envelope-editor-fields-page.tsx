...
import { AcroFormImportDialog, type TDetectedFieldWithItem } from '~/components/dialogs/acroform-import-dialog';
...
export const EnvelopeEditorFieldsPage = () => {
  ...
  const { envelope, editorFields, navigateToStep, editorConfig, setLocalEnvelope } = useCurrentEnvelopeEditor();
  ...
  const { mutateAsync: dismissDetectedFields } = trpc.envelope.item.dismissDetectedFields.useMutation();
  ...
  const calculatedReferenceData = useMemo(() => {
    const otherFields = editorFields.localFields.filter((field) => field.formId !== selectedField?.formId);
