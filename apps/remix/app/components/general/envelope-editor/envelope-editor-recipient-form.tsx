...
export const EnvelopeEditorRecipientForm = () => {
  ...
  // Normalizes signing orders into contiguous slots starting at 1 while preserving ties:
  // recipients sharing the same signing order stay grouped in the same slot (parallel
  // signing within a sequential order), and gaps are removed. Unmodifiable recipients
  // keep their existing signing order.
  const normalizeSigningOrders = (signers: typeof watchedSigners) => {
    return normalizeRecipientSigningSlots(signers, (signer) => canRecipientBeModified(signer.id));
  };

  // Whether the given signer is in the last signing order slot. Slots can be shared, so
  // this compares against the highest slot rather than the last position in the list.
  const isSignerInLastSlot = (signers: typeof watchedSigners, formId: string) => {
    const signer = signers.find((s) => s.formId === formId);

    return signer?.signingOrder === Math.max(...signers.map((s) => s.signingOrder ?? 0));
  };
  ...
  const onDragEnd = useCallback(
    async (result: DropResult) => {
      ...
      // Dragging assigns slots by list position, so any shared slots become sequential.
      const updatedSigners = normalizeRecipientSigningOrders(items, (signer) => canRecipientBeModified(signer.id));
  ...
  const handleRoleChange = useCallback(
    (index: number, role: RecipientRole) => {
      ...
      // Only change the role here. Normalizing preserves each recipient's signing order
      // slot (including shared slots) and only handles CC recipients gaining/losing an order.
      ...
      if (role === RecipientRole.ASSISTANT && isSignerInLastSlot(updatedSigners, currentSigners[index].formId)) {
        toast({
          title: t`Warning: Assistant as last signer`,
          description: t`Having an assistant as the last signer means they will be unable to take any action as there are no subsequent signers to assist.`,
        });
  ...
  const handleSigningOrderChange = useCallback(
    (index: number, newOrderString: string) => {
      ...
      if (isCcRecipient(signer) || !canRecipientBeModified(signer.id)) {
        return;
      }
      ...
      if (signer.role === RecipientRole.ASSISTANT && isSignerInLastSlot(updatedSigners, signer.formId)) {
        toast({
          title: t`Warning: Assistant as last signer`,
          description: t`Having an assistant as the last signer means they will be unable to take any action as there are no subsequent signers to assist.`,
        });
