...
import { AlertTriangleIcon, CalendarClock, InfoIcon } from 'lucide-react';
...
export const EnvelopeDistributeDialog = ({
  ...
  const { mutateAsync: cancelScheduledSend } = trpcReact.envelope.cancelScheduledSend.useMutation();
                        ...
                        <Form {...form}>
                          ...
                          >
                            ...
                            <div className="flex flex-col gap-y-3 rounded-lg border p-3">
                              <label className="flex cursor-pointer items-center gap-x-2 font-medium text-sm">
                                <Checkbox
                                  checked={isScheduled}
                              ...
                              {isScheduled && (
                                <div className="flex flex-col gap-y-2">
                                  ...
                                  <p className="text-muted-foreground text-xs">
                                    <Trans>
                                      The envelope will be sent automatically at this time, in your local timezone.
                                    </Trans>
                                  </p>

                                  {scheduleError && <p className="text-destructive text-xs">{scheduleError}</p>}
                                </div>
                              )}
                              ...
                              {envelope.scheduledAt && (
                                <Button
                                  ...
                                  variant="ghost"
                                  size="sm"
                                  className="h-auto self-start p-0 text-destructive text-xs hover:bg-transparent"
              ...
              {match(invalidEnvelopeCode)
                ...
                .with('MISSING_SIGNATURES', () => (
                  <AlertDescription>
                    ...
                    <p className="mt-3">
                      <Trans>
                        Assign at least one field to each signer — a signature field if they need to sign, or any other
                        field type (e.g. Text) if they only need to fill in data. If a recipient only needs to receive
                        or view the document, change their role to Viewer or CC instead.
                      </Trans>
                    </p>
                  </AlertDescription>
                ))
