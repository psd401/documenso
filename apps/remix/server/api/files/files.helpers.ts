...
import { addDraftStampToPdf } from '@documenso/lib/server-only/pdf/add-draft-stamp-to-pdf';
...
const handlePendingFileRequest = async ({
  ...
  let pdf = await generatePartialSignedPdf({
    pdfData: file,
    fields,
  });

  // Stamp the pending download so it cannot be mistaken for a completed/sealed
  // document (FI-25). Reload after generation so the stamp layers on top of
  // the fully flattened content.
  const stampedDoc = await PDF.load(pdf);
  await addDraftStampToPdf(stampedDoc);
  pdf = await stampedDoc.save({ useXRefStream: true });
